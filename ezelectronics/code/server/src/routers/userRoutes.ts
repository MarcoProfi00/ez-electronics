...
import * as userErrors from "../errors/userError"
/**
    ...
    initRoutes() {
            ...
            (req: any, res: any, next: any) => {
                if(req.user.username !== req.params.username) {
                    if(req.user.role !== "Admin")
                        return res.status(401).json({err: new userErrors.UnauthorizedUserError(), status: 401});
                }
        ...
        this.router.delete(
            ...
            (req: any, res: any, next: any) =>{ 
                if(req.user.username !== req.params.username) {
                    if(req.user.role !== "Admin")
                        return res.status(401).json({err: new userErrors.UserNotAdminError(), status: 401});
                }
        ...
        this.router.patch(
            ...
            this.errorHandler.validateRequest,
            (req: any, res: any, next: any) => {
                if (dayjs(req.body.birthdate).isAfter(dayjs())) {
                    return res.status(400).json({ error: "birthday cannot be in the future", status: 400 });
                }
