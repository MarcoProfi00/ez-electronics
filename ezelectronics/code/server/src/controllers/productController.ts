...
class ProductController {
    ...
    async registerProducts(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null) /**:Promise<void> */ {
        return this.dao.addNewProduct(model, category, quantity, details, sellingPrice, arrivalDate);
    }
    ...
    async getProducts(grouping: string | null, category: string | null, model: string | null) /**Promise<Product[]> */ {
        if (grouping == null || grouping == undefined) {
            return this.dao.getAllProducts(false);
        } else if (grouping == "category") {
            return this.dao.getProductsByCategory(category, false);
        } else if (grouping == "model") {
            return this.dao.getProductByModel(model, false);
        }
    }
    ...
    async getAvailableProducts(grouping: string | null, category: string | null, model: string | null) /**:Promise<Product[]> */ {
        if (grouping == null || grouping == undefined) {
            return this.dao.getAllProducts(true);
        } else if (grouping == "category") {
            return this.dao.getProductsByCategory(category, true);
        } else if (grouping == "model") {
            return this.dao.getProductByModel(model, true);
        }
    }
